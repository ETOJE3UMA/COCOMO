...
function ECalc() {
    ...
    for(let i = 1; i < 6; i = i + 1) {
		...
		console.log(`Значение: ${val}; Сумма: ${sum}`);
	}
...
function TMCalc() {
    ...
    let C = 3.67;
    let D = 0.28;
    return (SCED * C * Math.pow(PMWsced, D + 0.2 * (ECalc() - B))).toFixed(1);
}

function PMCalc(Size, ch) { //A - parametr
    multiply = 1;
    for(let i = 6; i < 13; i = i + 1) {
		let sel = document.getElementById(`sel${i}`);
		let val = sel.options[sel.selectedIndex].value;
    ...
    return (multiply * A * Math.pow(Size, ECalc())).toFixed(1);
}
